...
Provides a generic Fourier-mode implementation of the operators $J_{m,n}$ and $H_{m,n}$ using complex exponentials.
...
```math
f = J_{2,2}^*g_0 = -J_{2,2}g_0,
```

applies $H_{2,2}$, and evaluates

```math
\langle H_{2,2}f,f\rangle_{L^2}.
```
...
The operators $J_{2,2}$, $H_{2,2}$, and hence $L_{2,2}$, preserve this space. Moreover, $H_{2,2}$ is invertible on $X$ and has exactly one negative direction there, generated by
...
```math
\langle H_{2,2}f,f\rangle_{L^2}<0.
```

In this rank-one invariant block, the two properties

```math
f\in(\ker J_{2,2})^\perp
\qquad\text{and}\qquad
\langle H_{2,2}f,f\rangle_{L^2}<0
```
...
```math
-J_{2,2}g_{\mathrm{script}}
=
J_{2,2}g_{\mathrm{report}}.
```
...
- `applyJ` applies the Fourier action of $J_{m,n}$, shifting each frequency $(j,k)$ to combinations of $(j\pm m,k\pm n)$;
